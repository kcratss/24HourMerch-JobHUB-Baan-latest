...
namespace KEN.Reports
{
    public partial class InvoicedReport : System.Web.UI.Page
    {
        ...
        public void GeneratedReport(string fromdate,string todate)
        {
            ...
            InvoicedReportReportViewer.Visible = true;
            InvoicedReportReportViewer.LocalReport.Refresh();
            InvoicedReportReportViewer.ProcessingMode = ProcessingMode.Local;

            //date = date.Date;

            //string acf = Convert.ToDateTime(date);
            var Invoiced = dbContext.Pro_InvoicedReport(fromdate,todate).ToList();

            InvoicedReportReportViewer.LocalReport.ReportPath = Server.MapPath("~/Reports/InvoicedReport.rdlc");
            InvoicedReportReportViewer.LocalReport.DataSources.Clear();

            ReportDataSource rds = new ReportDataSource("InvoicedReport", Invoiced);
            ...
            this.InvoicedReportReportViewer.LocalReport.SetParameters(new ReportParameter[] { rpt1,rpt2,rpt3 });

            InvoicedReportReportViewer.LocalReport.DataSources.Add(rds);
            InvoicedReportReportViewer.LocalReport.Refresh();
            InvoicedReportReportViewer.DataBind();
        }
